...
def remap(zero,l_time, src_file, src_varname, src_grd, dst_grd, dmax=20, cdepth=0, kk=0,dst_dir='./'):
    ...
    pyroms_toolbox.nc_create_roms_file_3(dst_file, dst_grd, nctime)
	...
	time = cdf.variables['time'][zero:l_time]
	src_var = np.zeros((len(time),112,82))
	#src_var = np.zeros((len(time),142,137))
    time=time-time[0]
    if src_varname == 'ssh':
        Bpos = 't'
        Cpos = 'rho'
        ...
        wts_file = 'remap_weights_PUSSY_to_FINER_bilinear_t_to_rho.nc'
        dst_varname = 'temp'
        dimensions = ('ocean_time', 'sc_r', 'erho', 'xrho')
    ...
    if ndim == 4:
        dst_var=np.zeros((l_time-zero, 15, Mp,Np))
	for i in range(len(time)):
        ...
        dst_var=np.zeros((l_time-zero, Mp,Np))
	for i in range(len(time)):
		#src_varz = src_var[i,:,:]
		src_varz = np.zeros((len(src_var[:,0,0]),len(src_var[0,:,0]),len(src_var[0,0,:])))
