...
def remap_nested_uv(number,tart, tend, src_file, src_grd, dst_grd, dmax=20, cdepth=0, kk=0, dst_dir='./'):
    nctime.long_name = 'time'; nctime.units = 'seconds since 2004-01-01 00:00:00'; Mp,Np = dst_grd.vgrid.h.shape
    dxy=5; cdepth=0; kk=0; Bpos = 't'; Cpos = 'rho'; spval = -32767
    cdf = Dataset(src_file)
    Mp, Lp = dst_grd.hgrid.mask_rho.shape
    if number==1:
    	dst_file = 'windTRANSCHILD.nc'
    	pyroms_toolbox.nc_create_roms_file_2(dst_file, dst_grd, nctime)
    	cdf = Dataset(src_file, 'a', format='NETCDF3_64BIT')
    	l_time=tend-tart
	...
	time = time
  	...
  	wts_file = 'remap_weights_PUSSY_to_TRANSCHILD_bilinear_t_to_rho.nc'
	nc = Dataset(dst_file, 'a', format='NETCDF3_64BIT')
 	...
 	nc.variables['wind_time'].long_name = 'wind time'
  	nc.variables['wind_time'].units = 'days since 2010-01-01 00:00:00'
	...
	time_2=time
  	...
  	nc.createVariable('Uwind', 'f4', ('wind_time', 'erho', 'xrho'), fill_value=spval)
	...
	nc.createVariable('Vwind', 'f4', ('wind_time', 'erho', 'xrho'), fill_value=spval)
	...
	src_angle = pyroms.remapping.remap(src_grd.angle,  'remap_weights_PUSSY_to_TRANSCHILD_bilinear_t_to_rho.nc', spval=spval)
  	dst_angle = dst_grd.hgrid.angle_rho
  	angle = dst_angle - src_angle
   	angle = np.tile(angle, (dst_grd.vgrid.N, 1, 1))
   	eitheta = np.exp(-1j*angle[:,:,:])
   	dst_u=np.zeros((l_time, Mp, Np))
   	dst_v=np.zeros((l_time, Mp, Np))
	...
	print 'surface wind is written into the separated netcdf file'
   	nc.close()
    if number==2:
    	dst_file = 'momentumTRANSCHILD.nc'
    	pyroms_toolbox.nc_create_roms_file_2(dst_file, dst_grd, nctime)
    	cdf = Dataset(src_file, 'a', format='NETCDF3_64BIT')
    	l_time=tend-tart
    	time = (cdf.variables['time'][tart:tend])
	...
	nc = Dataset(dst_file, 'a', format='NETCDF3_64BIT')
 	...
 	nc.variables['sms_time'].long_name = 'wind time'
  	nc.variables['sms_time'].units = 'days since 2010-01-01 00:00:00'
  	nc.variables['sms_time'].field = 'sms_time field'
  	nc.variables['sms_time'][:] = time/24.
  	nc.createVariable('sustr', 'f4', ('sms_time', 'erho', 'xrho'), fill_value=spval)
	...
	#nc.variables['sustr'].coordinates = 'lon lat'
	nc.createVariable('svstr', 'f4', ('sms_time', 'erho', 'xrho'), fill_value=spval)
