...
nc = Dataset('/home/eivanov/coawst_data_prrocessing/Temporal/Input_files_to_ROMS/Rivers_parent.nc', 'w', format='NETCDF4')
nc.Description = 'Discharges of major rivers'
nc.Author = 'Evgeny Ivanov'
nc.Created = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
nc.title = 'Discharges of major rivers'
nc.createDimension('river_time', 365*3)					###
####nc.createDimension('river_time', 365)
nc.createDimension('xi_rho', 67)
nc.createDimension('xi_u', 66)
nc.createDimension('xi_v', 67)
nc.createDimension('eta_rho', 89)
nc.createDimension('eta_u', 89)
nc.createDimension('eta_v', 88)
...
M=MM[365:730]; D=DD[365:730]; Q=np.zeros((365)); n=0; time=np.zeros((365*3))
...
nc.variables['river_time'].units = "days since 2006-01-01 00:00:00"
nc.variables['river_time'][:]=np.arange(365*3)	###
...
nc.variables['river'].long_name = "river runoff identification number: 1)Thames, 2)Scheldt, 3)Rhine+Maas 4)Seine"
nc.variables['river'][:]=[1,2,3]#,4]
...
nc.variables['river_Xposition'][:]=[36,1,13]#,11]
...
'''
Thames_t=np.zeros((365,20))
Scheldt_t=np.zeros((365,20))
Maas_t=np.zeros((365,20))
Rhine_t=np.zeros((365,20))
#Seine_t=np.zeros((365,15))
'''
...
nc.variables['river_transport'][:,0]=np.concatenate((-Thames,-Thames,-Thames),axis=0)
nc.variables['river_transport'][:,1]=np.concatenate((Scheldt,Scheldt,Scheldt),axis=0)
nc.variables['river_transport'][:,2]=np.concatenate((Rhine+Maas,Rhine+Maas,Rhine+Maas),axis=0)
...
nc.variables['river_temp'][:,:,0]=np.concatenate((Thames_t,Thames_t,Thames_t),axis=0)
nc.variables['river_temp'][:,:,1]=np.concatenate((Scheldt_t,Scheldt_t,Scheldt_t),axis=0)
nc.variables['river_temp'][:,:,2]=np.concatenate((0.15*Maas_t+0.85*Rhine_t,0.15*Maas_t+0.85*Rhine_t,0.15*Maas_t+0.85*Rhine_t),axis=0)

for i in range(Nr):
	nc.variables['river_salt'][:,:,i]=np.ones((365*3,20))
...
#copyfile('Rivers.nc', '/home/eivanov/COAWST/Data/ROMS/Rivers/Rivers_10_years_salt_0.nc')
